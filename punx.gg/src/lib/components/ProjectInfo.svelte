<script lang="ts">
	import ProjectTag from './ProjectTag.svelte';

	export let title: string = 'Project';
	export let tags: ProjectTag[] = [];
	export let description: string = 'description';
	export let icon: string = 'path/to/icon';
	export let githubUrl: string = '#';
</script>

<a href={githubUrl} target="_blank" rel="noopener noreferrer" class="group block">
	<div
		class="
      relative
      flex items-center
      justify-between
      rounded-2xl
      border
      border-white/10
      bg-black
      px-6
      py-4
      transition
      duration-200
      hover:border-white/25
      hover:bg-white/5
    "
	>
		<!-- Left content -->
		<div class="flex items-start gap-4">
			<!-- Icon cluster -->
			<div class="relative h-8 w-8 shrink-0">
				<!-- Cursor / pointer icon -->
				<svg
					class="absolute left-0 top-0 h-5 w-5 text-white"
					viewBox="0 0 24 24"
					fill="currentColor"
				>
					<path d="M3 2l7.5 18 2.5-7 7-2.5L3 2z" />
				</svg>

				<!-- Small square icon -->
				<div
					class="
            absolute
            right-0
            bottom-0
            flex
            h-4
            w-4
            items-center
            justify-center
            rounded-sm
            border
            border-white/30
            text-[10px]
            text-white/80
          "
				>
					▢
				</div>
			</div>

			<!-- Text -->
			<div class="flex flex-col">
				<h3 class="text-lg font-semibold tracking-tight">
					{title}
				</h3>
				<p class="mt-0.5 text-sm leading-snug">
					{description}
				</p>
			</div>
		</div>

		<!-- Right GitHub icon -->
		<div
			class="
        ml-4
        flex
        h-9
        w-9
        items-center
        justify-center
        rounded-full
        border
        border-white/20
        text-white/80
        transition
        duration-200
        group-hover:border-white/40
        group-hover:text-white
      "
			aria-hidden="true"
		>
			<svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
				<path
					fill-rule="evenodd"
					d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.009-.868-.014-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.071 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844a9.59 9.59 0 012.504.337c1.909-1.296 2.747-1.026 2.747-1.026.546 1.378.202 2.397.1 2.65.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.944.359.31.678.92.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.523 2 12 2z"
					clip-rule="evenodd"
				/>
			</svg>
		</div>
	</div>
</a>
